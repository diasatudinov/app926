...
//                                .padding(.bottom, 20)
                ...
                Button {
                    if !name.isEmpty && !clientName.isEmpty && !phoneNumber.isEmpty && !productQuantity.isEmpty && !adress.isEmpty && !price.isEmpty && !notes.isEmpty {
                        ...
                        viewModel.editDessert(order, newName: name, newImage: selectedImage, clientName: clientName, phone: phoneNumber, quantity: productQuantity, adress: adress, price: price, newDate: date, newNote: notes)
                        presentationMode.wrappedValue.dismiss()
                    }
...
#Preview {
    EditOrderView(viewModel: OrdersViewModel(), order: Order(name: "Lambeth-style cake", clientName: "Ralph Edwards", phoneNumber: "(316) 555-0116", productQuantity: "1 pc", adress: "8502 Preston Rd. Inglewood, Maine 98380", price: "$250", date: Date(), time: Date(), notes: "White cream cake. Design in the form of a frame of red ribbons and an inscription in the middle. Inscription: 23 years"))
}
